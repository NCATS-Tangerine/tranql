...
On a final contextual note, we've also implemented a GraphQL interface to federated data services. GraphQL, it's name not withstading, does not provide much in the way of constructs allowing the user to think explicitly in terms of a knowledge graph as, say, Cypher. And, again, it's query syntax is highly unfamiliar to clinical data and medical communities.
